...
export type CompassAction =
  ...
  switch (action.type) {
    case 'setCompass': {
      ...
      const compass = {
        ...action.compass,
        radius: Math.max(COMPASS_MIN_LEG_LENGTH, action.compass.radius),
      };
...
// --- Geometry helpers ---
export const COMPASS_PIN_RADIUS = 8;
export const COMPASS_PENCIL_RADIUS = 8;
export const COMPASS_LEG_RADIUS = 8;
export const COMPASS_PENCIL_LENGTH = 24;
export const COMPASS_MIN_LEG_LENGTH = 40;

// Get positions of handles
export function getCompassPencil(compass: Compass): Point {
  return {
    x: compass.pin.x + COMPASS_PENCIL_LENGTH * Math.cos(compass.angle),
    y: compass.pin.y + COMPASS_PENCIL_LENGTH * Math.sin(compass.angle),
  };
}
export function getCompassLeg(compass: Compass): Point {
  return {
    x:
      compass.pin.x +
      compass.radius * compass.legOffset * Math.cos(compass.angle),
    y:
      compass.pin.y +
      compass.radius * compass.legOffset * Math.sin(compass.angle),
  };
}

// Hit test helpers
export function isPointInCompassPin(
  x: number,
  y: number,
  compass: Compass,
): boolean {
...
export function isPointInCompassPencil(
  x: number,
  y: number,
  compass: Compass,
): boolean {
...
export function isPointInCompassLeg(
  x: number,
  y: number,
  compass: Compass,
): boolean {
